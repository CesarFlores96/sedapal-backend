...
isolation. The FastAPI service layer (``app/services/billing.py``) is
responsible for loading the billing history and operational context and for
handing normalized structures to :func:`analyze_supply_consumption`.

Methodology (see the project plan):
...
centralized and configurable, matching the existing backend conventions
(``compute_safe_variation_percent`` / ``MIN_MEDIAN_COVERAGE_RATIO`` from
...
def compute_safe_variation_percent(target: float | None, baseline: float | None) -> float | None:
    """Percent variation with the same zero-baseline convention already used in
    ``app/repositories/reports.py`` (baseline 0 -> 0 if target 0 else 100)."""
...
def _operational_summary_for_period(
    ...
    return {
        ...
        # No documented column distinguishes "facturación especial" (e.g.
        # refacturación) or "estimación de consumo" in the current schema
        # (customer_debts.status is only vigente/pagada; reading_type is only
        # REMOTA/DIRECTA) -- report honestly rather than inventing a signal.
...
def _last_present_month(current_months: dict[int, float | None]) -> int | None:
    """Last month with a present billing record, **including a real zero** --
    a zero-consumption month is itself the near-zero anomaly we must analyse,
    so the anomaly focus uses this rather than ``_last_billed_month``."""
    present = [m for m, v in current_months.items() if v is not None]
...
def _build_insight_cards(
    ...
    """Reproduce the cards the frontend used to build client-side, returning a
    semantic ``tone`` instead of Tailwind classes so styling stays in the UI."""
...
def analyze_supply_consumption(
    ...
    *,
    ...
    Returns the extended, frontend-compatible contract with ``analysisByYear``
    for every year present so the UI can toggle years without refetching.
    """
    ...
    for year in years:
        ...
        months = [
            _analyze_month(monthly_water, year, month, context, min_obs, cutoff)
            for month in range(1, 13)
        ]
        ...
        # most recent first (matches the reference table ordering).
        periods = [
            {
                "year": m["year"],
                "month": m["month"],
